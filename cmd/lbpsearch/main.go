...
func main() {
	...
	r.Get("/", website.IndexHandler())
	r.Get("/search", website.SearchHandler(conn))
	r.Get("/user/{npHandle}", website.UserHandler(conn))
	r.Get("/slot/{slotID}", website.SlotHandler(conn))
	r.Get("/icon/{hash}", website.IconHandler(cfg))
	r.Get("/changelog", website.ChangelogHandler())
	r.Get("/dl_archive/{id}", website.DownloadArchiveHandler(conn, cfg))
	r.Get("/sitemap_other.xml", website.SitemapHandler())
	r.Get("/sitemap_other.xml.gz", website.SitemapGZHandler())
	r.Get("/sitemap.xml", website.SitemapIndexHandler())
	r.Get("/robots.txt", website.RobotsTXTHandler())
